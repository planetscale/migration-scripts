...
**filters.ini awareness:** The migration only copies the subset of objects allowed by `~/filters.ini`, so this script scopes every check to that same subset — otherwise intentionally-excluded objects (e.g. Supabase's `auth`, `storage`, `realtime` schemas) report as "missing in target" false positives, and the exact-count sampler can even flag an excluded table as confirmed data loss. It honors `[exclude-schema]`/`[include-only-schema]` (all checks), `[exclude-table]`/`[include-only-table]` (table, column, index, constraint, row-count, spot-check, and exact-count checks), and `[exclude-extension]` (every check, not just the extensions one), using the same interpretation as `preflight-check.sh`. The active scope is printed in the header. filters.ini is **required** — verify uses the same scope the migration ran with, so it errors out if the file is absent; to compare everything, use a filters.ini with no scope sections. With no scope sections every clause collapses to the empty string, so the SQL is identical to a filter-unaware comparison.

**Dependency scoping:** pgcopydb never creates an object whose parent was filtered out, so verify also excludes objects that are *indirectly* out of scope. Without this they are the bulk of the false alarms, because the object's own name matches nothing in filters.ini:

- a sequence owned by an excluded table (`serial`/identity columns) — standalone sequences are never dropped by this rule
- a foreign key whose *referenced* table is excluded, even when the FK's own table is fully in scope (typical for a cross-schema FK into an excluded schema)
- a view reading an excluded relation, transitively — a view on an excluded table takes the view built on *that* view with it. The columns check needs the same rule, because `information_schema.columns` reports view columns too; without it a dependency-excluded view reappears as "missing columns"
- everything an `[exclude-extension]` entry owns: its tables, columns, indexes, constraints, views, sequences and routines

These exclusions are silent — there is no per-object "skipped" accounting. Objects that *were* in scope and are genuinely absent still FAIL.

**`include-only-table` skips:** in this mode pgcopydb copies the listed tables plus their own indexes, constraints and sequences, but not the views, routines or standalone sequences that merely share those schemas. Comparing those is guaranteed to report false alarms, so checks 6 (views), 7 (functions) and 9 (sequences) are skipped with the reason printed in the section header. Everything else stays scoped to the listed tables.

**Filter sanity:** verify WARNs if filters.ini uses a pgcopydb-disallowed section combination (via `filter_conflicts`), and if it uses a section outside the six documented ones (e.g. `[exclude-table-data]`, `[exclude-index]`) — those are surfaced rather than half-honoured, since the derived scope may not match what actually migrated.
...
**Key functions:** `parse_filters_ini <path>` (populates `FILTER_*` arrays from `[exclude-schema]`, `[exclude-table]`, `[include-only-schema]`, `[include-only-table]`, `[exclude-extension]`; `[exclude-event-trigger]` is recognised but needs no scoping, and any other section lands in `FILTER_UNKNOWN_SECTIONS`), `filter_scope_mode` (precedence: `include-only-table` > `include-only-schema` > `exclude-schema` > `all`), `schema_clause <col>` / `table_clause <schema-col> <rel-col>` / `extension_clause <col>` (emit `AND ... IN/NOT IN (...)` fragments for any catalog column), `filter_scope_describe` (header summary), `filter_conflicts` (pgcopydb-disallowed section combinations), and `filter_unknown_sections` (sections the library does not model).

**Dependency-scope functions:** `extension_rel_clause <schema-col> <rel-col>` and `extension_oid_clause <catalog> <oid-col>` (objects owned by an excluded extension — the name-keyed form works against `pg_class`, `information_schema.columns`, `pg_indexes`, `pg_views` and `pg_sequences`; the oid-keyed form covers `pg_proc` and `pg_constraint`), `fk_target_clause <confrelid-col>`, `sequence_owner_clause <schema-col> <rel-col>`, and `view_dep_clause <schema-col> <rel-col>` (recursive, so a view on a view on an excluded table is caught). All are built from one internal out-of-scope predicate, so a new reason for exclusion only has to be added in one place. **Every clause returns the empty string when the filter has nothing to exclude** — that is what keeps an empty filters.ini byte-identical to a filter-unaware comparison, and it is the invariant the tests defend hardest.

**Tests:** `filters-lib.test.sh` (helpers in isolation), `verify-migration.query.test.sh` (extracts each `*_QUERY` from `verify-migration.sh` and evaluates it, so a helper that exists but was never wired into a check is caught), and `verify-migration.e2e.test.sh` (two `postgres:16` containers: `EXPLAIN`s every built query in every filter mode, then builds each target the way pgcopydb would have left it and asserts verify's exit code — including that genuine drift still exits 2). The e2e suite runs verify under an isolated `HOME` so it never reads the developer's `~/.env` or `~/filters.ini`.
