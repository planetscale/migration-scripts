...
**Dependency scoping:** pgcopydb never creates an object whose parent was filtered out, so verify also excludes objects that are *indirectly* out of scope. These are the bulk of the false alarms, because the object's own name matches nothing in filters.ini:

- a sequence owned by an excluded table (`serial`/identity); standalone sequences are never dropped by this rule
- a foreign key whose *referenced* table is excluded, even when the FK's own table is fully in scope — typically a cross-schema FK into an excluded schema
- a view reading an excluded relation, transitively: a view on an excluded table takes the view built on *that* view with it. The columns check needs the same rule, since `information_schema.columns` reports view columns and would otherwise return them as "missing columns"
- everything an `[exclude-extension]` entry owns: its tables, columns, indexes, constraints, views, sequences and routines

These exclusions are silent — no per-object "skipped" accounting. Objects that *were* in scope and are genuinely absent still FAIL.

**`include-only-table` skips:** in this mode pgcopydb copies the listed tables plus their own indexes, constraints and sequences, but not the views, routines or standalone sequences that merely share those schemas. Comparing those would false-alarm, so checks 6 (views), 7 (functions) and 9 (sequences) are skipped, with the reason in the section header. Everything else stays scoped to the listed tables.
...
**Dependency-scope functions:** `extension_rel_clause <schema-col> <rel-col>` and `extension_oid_clause <catalog> <oid-col>` (objects owned by an excluded extension — the name-keyed form works against `pg_class`, `information_schema.columns`, `pg_indexes`, `pg_views` and `pg_sequences`; the oid-keyed form covers `pg_proc` and `pg_constraint`), `fk_target_clause <confrelid-col>`, `sequence_owner_clause <schema-col> <rel-col>`, and `view_dep_clause <schema-col> <rel-col>` (recursive, so a view on a view on an excluded table is caught). All are built from one internal out-of-scope predicate, so a new reason for exclusion is added in one place. **Every clause returns the empty string when the filter has nothing to exclude** — that is what keeps an empty filters.ini byte-identical to a filter-unaware comparison.
